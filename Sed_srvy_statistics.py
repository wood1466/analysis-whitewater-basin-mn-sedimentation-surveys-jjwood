...
Bsn = 1  # Defines variable as integer. Sets binary toggle.
...
rvrs = ['Main Stem', 'Middle Fork', 'North Fork', 'South Fork', 'Trout Creek', 'Beaver Creek', 'Logan Creek', 'Dry Creek']
rvr_strt = rvrs[0]
rvr_end = rvrs[1]
...
if rvr_nly == 1:  # Conditional statement. Executes lines if condition satisfied.
    rvr_chnls = [rvr_strt]  # Defines list. One element to enable analyses of single survey range.
    rvr_end = rng_strt  # Redefines variable as integer. For display.
elif rvr_nly == 0:  # Conditional statement. Executes lines if condition satisfied.
    # rvr_chnls = [rvr_strt:rvr_end].
...
if Ststcs == 1:
                        ...
                        plot_qq(0, sed_rts_arry, '45', 'Average sedimentation rates', 1, 3)
                        plt.figure(0)
                        plt.close()
                        plot_histogram(0, sed_rts_arry, 'Cyan', 'Black', 'Average sedimentation rates', 1, 3)
                        plt.figure(0)
                        plt.close()
                        plot_qq(0, dpth_arry, '45', 'Average sedimentation depth', 1, 3)
                        plt.figure(0)
                        plt.close()
                        plot_histogram(0, dpth_arry, 'Cyan', 'Black', 'Average sedimentation depth', 1, 3)
                        plt.figure(0)
                        plt.close()
# ======================================================================================================================
# END ------------------------------------------------------------------------------------------------------------------
