...
# Data
inpt_fl1 = inpt_fldr + '/Whitewater sedimentation survey data 4 15.csv'  # Defines variable. Sets file path to input
# file.
inpt_fl2 = inpt_fldr + '/Monument metadata semifinal 4 11.csv'  # Defines variable. Sets file path to input file.
inpt_fl3 = inpt_fldr + '/Monument coordinates semifinal 4 15.csv'  # Defines variable. Sets file path to input file.

# Limits of analysis
rng_strt = 1  # Defines variable. Sets start survey range number for operation loop.
rng_end = 107  # Defines variable. Sets end survey range number for operation loop.
...
for i in rng_nmbrs:  # Begins loop. Loops through array elements. Loops through transect numbers. Calculates coordinates
    ...
    if len(df_mnt_crdnts.axes[0]) != df_mnt_crdnts.isnull().sum().sum():  # Begins conditional statement. Checks
    # inequality.
        ...
        # Check for shift in monument coordinates ----------------------------------------------------------------------

        mnt1_e, mnt1_n = check_marker_shift(mnt1_e, mnt1_n, df_mnt_crdnts, 0, ft_to_m, 0)  # Redefines variables. Calls
        # function. Checks if coordinates need shifted to account for monument offset from range line.
        mnt2_e, mnt2_n = check_marker_shift(mnt2_e, mnt2_n, df_mnt_crdnts, 1, ft_to_m, 0)  # Redefines variables. Calls
        # function. Checks if coordinates need shifted to account for monument offset from range line.

    # Select survey data -----------------------------------------------------------------------------------------------

    srvy_nmbrs = slice_DataFrame_columns('Array', 'Integer', df_rng_dt, 'survey_number', 1, 0, 0)  # Defines array.
    ...
    for j in srvy_nmbrs:  # Begins loop. Loops through array elements. Loops through survey numbers. Calculates
        ...
        df_rng_srvy_dt = slice_DataFrame_rows('Equals', df_rng_dt, 'survey_number', j, 0)  # Defines DataFrame. Calls
        # function. Slices DataFrame to yield single range single survey data.

        # Metadata
        df_rng_srvy_mtdt = slice_DataFrame_rows('Equals', df_rng_mtdt, 'survey_number', j, 0)  # Defines DataFrame.
        ...
        if len(df_elvtns.axes[0]) != df_elvtns.isnull().sum().sum():  # Begins conditional statement. Checks inequality.
            ...
            # Survey azimuth
            thta = range_orientation_calculator(mnt1_e, mnt2_e, mnt1_n, mnt2_n, 0)  # Defines variable. Calls
            ...
            # # check.
            # plt.scatter([mnt1_e, mnt2_e], [mnt1_n, mnt2_n], c='m', alpha = 0.9)  # Creates scatter plot. Plots monument
        ...
        if i == rng_end:  # Begins conditional statement. Checks equality. Executes code when condition satisfied.
            ...
            if j == srvy_nmbrs[-1]:  # Begins conditional statement. Checks equality. Executes code when condition
                ...
                srvy_eras = slice_DataFrame_columns('Array', 'Integer', df_srvy_dt, 'survey_era', 1, 0, 0)  # Defines
                # array. Calls function. Slices DataFrame to yield survey eras of complete dataset.

                for k in srvy_eras:  # Begins loop. Loops through survey era years. Digitizes data for each survey
                    # era in sequence.
                    df_srvy_era_k = slice_DataFrame_rows('Equals', df_srvy_dt, 'survey_era', k, 0)  # Defines
                    # DataFrame. Calls function. Slices DataFrame to yield single survey era data.

                    gdf_srvy_era_k = gpd.GeoDataFrame(df_srvy_era_k,
                                                      geometry=gpd.points_from_xy(df_srvy_era_k.easting_m,
                                                                                  df_srvy_era_k.northing_m),
