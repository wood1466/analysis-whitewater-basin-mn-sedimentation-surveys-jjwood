...
# Data
inpt_fl1 = inpt_fldr + '/Survey data new MS-MF.csv'  # Defines variable. Sets file path to input file.
...
rng_end = 26  # Defines variable. Sets end survey range number for operation loop.
...
for i in rng_nmbrs:  # Begins loop. Loops through array elements. Loops through transect numbers. Calculates coordinates
    ...
    for j in srvy_nmbrs:  # Begins loop. Loops through array elements. Loops through survey numbers. Calculates
        ...
        df_elvtns = slice_DataFrame_columns('DataFrame', 'Float', df_rng_srvy_dt, 'Elv_geo_ft', 0, 0, 0)  # Defines
        ...
        if len(df_elvtns.axes[0]) != df_elvtns.isnull().sum().sum():  # Begins conditional statement. Checks inequality.
            # Digitizes all data that has elevation records. Ignores soil bore depth.
            ...
            # check.
            # plt.scatter([mnt1_e, mnt2_e], [mnt1_n, mnt2_n], c='Orange')  # Creates scatter plot. Plots monument
            # coordinates. Enable for check.
            # plt.pause(0.1)  # Plot pause command. Enables continuous plotting on same figure at interval of 1 second.
        ...
        if i == rng_end:  # Begins conditional statement. Checks equality. Executes code when condition satisfied.
            # Saves coordinates to DataFrame and digitizes DataFrame when all surveys have been completed.
            if j == srvy_nmbrs[-1]:  # Begins conditional statement. Checks equality. Executes code when condition
                ...
                # Concatenates DataFrames. Appends coordinates to DataFrame for digitization.
                ...
                for k in srvy_eras:  # Begins loop. Loops through survey era years. Digitizes data for each survey
                    # era in sequence.
                    df_srvy_era_k = slice_DataFrame_rows('Equals', df_srvy_dt, 'Srvy_year', k, 0)  # Defines
                    # DataFrame. Calls function. Slices DataFrame to yield single survey era data.

                    gdf_srvy_era_k = gpd.GeoDataFrame(df_srvy_era_k,
                                                      geometry=gpd.points_from_xy(df_srvy_era_k.Easting,
                    ...
                    gdf_srvy_era_k.to_file(otpt_fldr + '/' + gis_fldr + gpkg, layer=lyr_nm, driver='GPKG',
                                           index=True)  # Saves file to directory.
