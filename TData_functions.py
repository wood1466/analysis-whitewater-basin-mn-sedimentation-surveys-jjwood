...
def CreateForwardRange(start, end, step, display):  # Defines function. For generating forward array between two numbers.
    ...
    if display == 1:  # Begins conditional statement. Checks equality. For display.
        print('\033[1mCREATED ARRAY:\033[0m \n  Limits: ' + str(start) + ' & ' + str(EndLabel) + ' --> Array:', Range)  # Displays objects.
...
def CalculateTransectAzimuth(x1, y1, x2, y2, display):  # Defines function. For transect azimuth calculations from reference coordinates.
    ...
    if Quadrant == 1:  # Begins conditional statement. Checks equality. Calculates transect azimuth.
        ReferenceAngle = (1/2) * np.pi  # Defines variable. Sets reference angle for correction of calculated orientation to azimuth.
        ...
        ReferenceAngle = (3/2) * np.pi  # Defines variable. Sets reference angle for correction of calculated orientation to azimuth.
        Azimuth = ReferenceAngle - abs(np.arctan(DeltaX / DeltaY))  # Defines variable. Calculates transect azimuth.
...
def CalculationExclusionChecker(dataframe, column1, column2, column3, column4, position, value, display):  # Defines function. For searching transect data for exclusions prior to calculations.
    dfExcCheck = SliceDataFrameRows('Equals', dataframe, column1, value, 0)  # Defines DataFrame. Calls function. Slices DataFrame to yield excluded transect data.
    if dfExcCheck.shape[0] != 0:  # Begins conditional statement. Checks equality. Selects exclusion points.
        TExcStart = SliceDataFrameCell('Float', dfExcCheck, position, column2, 0)  # Defines variable. Calls function. Slices DataFrame to yield exclusion point.
        TExcEnd = SliceDataFrameCell('Float', dfExcCheck, position, column3, 0)  # Defines variable. Calls function. Slices DataFrame to yield exclusion point.
        TExcPairYear = SliceDataFrameCell('String', dfExcCheck, position, column4, 0)  # Defines variable. Calls function. Slices DataFrame to yield applicable exclusion year.
    ...
    if display == 1:  # Begins conditional statement. Checks equality. For display.
        print('\033[1mRETRIEVED TRANSECT DATA DISCONTINUITY:\033[0m ' + str(TExcStart) + '-' + str(TExcEnd) + ' for ' +str(TExcPairYear) + '\n')  # Displays objects.
    return TExcStart, TExcEnd, TExcPairYear  # Ends function execution.


def MoveReferenceCoordinates(x, y, dataframe1, dataframe2, position, column1, column2, column3, conversion_factor, display):  # Defines function. For moving transect reference coordinates if established off of line.
    PointOffsetDistanceFt = SliceDataFrameCell('Float', dataframe1, position, column1, 0)  # Defines variable. Calls function.  Slices DataFrame to yield marker off transect distance.
    PointOffsetDistanceM = PointOffsetDistanceFt / conversion_factor  # Defines variable. Converts feet to meters.
    PointOffsetDirection = SliceDataFrameCell('String', dataframe1, position, column2, 0)  # Defines variable. Calls function.  Slices DataFrame to yield marker off transect direction.
    CoordinateAccuracyM = SliceDataFrameCell('String', dataframe2, position, column3, 0)  # Defines variable. Calls function.  Slices DataFrame to yield marker coordinate estimated accuracy.
    CoordinateAccuracyMList = CoordinateAccuracyM.split('-')  # Defines list. Splits string into component parts by delimiter.
    if len(CoordinateAccuracyMList) == 3:  # Begins conditional statement. Checks equality. Checks if estimated coordinate accuracy is range.
    ...
    if PointOffsetDistanceM > CoordinateAccuracyM:  # Begins conditional statement. Checks relation. Moves coordinates
        ...
        if PointOffsetDirection == 'North':  # Begins conditional statement. Checks equality. Moves coordinates by component.
            y -= PointOffsetDistanceM  # Redefines variable.
        elif PointOffsetDirection == 'South':  # Continues conditional statement. Checks equality. Moves coordinates by component.
            y += PointOffsetDistanceM  # Redefines variable.
        elif PointOffsetDirection == 'East':  # Continues conditional statement. Checks equality. Moves coordinates by component.
            x -= PointOffsetDistanceM  # Redefines variable.
        elif PointOffsetDirection == 'West':  # Continues conditional statement. Checks equality. Moves coordinates by component.
            x += PointOffsetDistanceM  # Redefines variable.
    ...
    if display == 1:  # Begins conditional statement. Checks equality. For display.
        print('\033[1mMARKER SHIFT:\033[0m\n Direction: ' + str(PointOffsetDirection) + '\n Magnitude: ' + str('%.2f' % PointOffsetDistanceM) + '\n Output coordinate: ' + str('%.2f' % x) + ', ' + str('%.2f' % y) + '\n')  # Displays objects.
    return x, y  # Ends function execution.


def SliceDataFrameCell(data_type, dataframe, position, column, display):  # Defines function. For DataFrame slicing by row value and index.
    ...
    if Type == data_type:  # Begins conditional statement. Checks equality. Checks if data type is desired by function input.
...
def SliceDataFrameColumns(output, data_type, dataframe, column, check_duplicates, drop_nan, display):  # Defines function. For DataFrame slicing by column.
    dfSlice = dataframe[column]  # Defines DataFrame. Slices DatFrame by column header.
    if check_duplicates == 1:  # Begins conditional statement. Checks equality. Checks if duplicate values should be removed from DataFrame.
        dfSlice = dfSlice.drop_duplicates(keep='first')  # Redefines DataFrame. Drops all duplicate values.
    if drop_nan == 1:  # Begins conditional statement. Checks equality. Checks if NaN values should be removed from DataFrame.
...
def SliceDataFrameRows(search_type, dataframe, column, value, display):  # Defines function. For DataFrame slicing by row value.
    if search_type == 'Equals':  # Begins conditional statement. Checks equality. Sets function format.
        ...
        dfSlice = dataframe[dataframe[column] != value]  # Defines DataFrame. Slices DataFrame by value and relation.
    dfSlice = dfSlice.loc[:, ~dfSlice.columns.str.match('Unnamed')]  # Redefines DataFrame. Searches for empty columns and deletes them.
    if display == 1:  # Begins conditional statement. Checks equality. For display.
        print('\033[1mCREATED DATAFRAME: \033[0m \n...\n', dfSlice, '\n')  # Displays objects.
    return dfSlice  # Ends function execution.